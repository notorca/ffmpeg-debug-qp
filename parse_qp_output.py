        ...
        for line in f:
            ...
            if "New frame" in line:
                if has_current_frame_data:
                    ...
                    frame_data = {"frameType": frame_type, "frameSize": frame_size, "timestamp": timestamp, "mb_width": mb_width}
            ...
            if "ts" in line:
                timestamp = int(re.findall(r'\d+', line[line.rfind("ts"):])[0])
            if "mb_width" in line:
                mb_width = int(re.findall(r'\d+', line[line.rfind("mb_width"):])[0])
